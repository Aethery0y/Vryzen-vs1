 ...
 * @param {string} jid - The JID (number@s.whatsapp.net)
...
function getContactName(jid) {
    ...
    const contact = db.contacts[normalizedNumber];
    if (contact && contact.name) {
        return contact.name;
    }
    
    // If we don't have a name, just return the number
    return number;
}

/**
 * Get a group's name
 * @param {string} jid - The JID (groupid@g.us)
 * @returns {string|null} The group name or null
 */
function getGroupName(jid) {
    if (!jid) return null;
    
    // We'll store group info in the data store
    const groups = getData('groups') || {};
    
    if (groups[jid] && groups[jid].name) {
        return groups[jid].name;
    }
    
    // If we don't have a name, just return the ID part
    return jid.split('@')[0];
}
