...
async function createNewGroup(sock, operationId) {
    try {
        // Check WhatsApp connection using our enhanced helper module
        const connectionStatus = connectionHelper.checkConnectionBeforeAction('create group', sock);
        if (!connectionStatus.success) {
            // Special handling for 403 errors
            if (connectionStatus.error403) {
                console.log("Cannot create group: WhatsApp connection blocked with 403 error", 
                    connectionStatus.detail?.errorLocation || '');
                
                return {
                    success: false,
                    message: "WhatsApp is currently blocking our connection with a 403 error. The system will automatically retry connecting. Please try again in a few minutes.",
                    error403: true,
...
async function processBatch(sock, operationId, batchId = null) {
    try {
        ...
        // Check if the WhatsApp connection is active using our enhanced helper module
        const connectionStatus = connectionHelper.checkConnectionBeforeAction('process invitation batch', sock);
        if (!connectionStatus.success) {
            // Special handling for 403 errors
            if (connectionStatus.error403) {
                console.error('WhatsApp connection blocked with 403 error for processBatch:', 
                    connectionStatus.detail?.errorLocation || '');
                ...
                return {
                    ...
                    error403: true,
                    detail: connectionStatus.detail,
                    // Return the batch ID so it can be retried
                    batchId
                };
            }
