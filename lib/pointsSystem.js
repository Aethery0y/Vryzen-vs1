 ...
 */
// This is the main function to get points - it's also exported as getPoints
...
/**
 * Alias for getUserPoints (used by external modules that expect 'getPoints')
 */
...
/**
 * Update user points (add or deduct) - general purpose function for external modules
 * @param {string} userId User's phone number or ID
 * @param {number} points Points to update (positive to add, negative to deduct)
 * @param {string} source Source of the update
 * @param {string} reason Description of the update
 * @returns {object} Result object with success, points, error
 */
function updatePoints(userId, points, source = 'generic', reason = '') {
    if (points > 0) {
        return addPoints(userId, points, source);
    } else if (points < 0) {
        return deductPoints(userId, Math.abs(points), reason || source);
    } else {
        // No points change
        return {
            success: true,
            currentPoints: getUserPoints(userId),
            message: 'No points were added or deducted.'
        };
    }
}

/**
 * Get user profile including points and stats
 * @param {string} userId User's phone number or ID
...
function getUserProfile(userId) {
    ...
    return {
        ...
        dailyStats: getUserDailyStats(userId),
...
function getUserRank(userId) {
    ...
    const userIndex = sortedUsers.findIndex(([id, _]) => id === userId);
    
    // Return rank (1-based) or 0 if user not found
 ...
 * @returns {Array} Array of user objects with ID, points, and rank
 */
