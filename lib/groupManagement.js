    ...
    try {
        ...
        try {
            ...
            if (!isAdmin) {
                ...
                await sock.sendMessage(groupId, {
                    text: `⚠️ For best results, please make the bot an admin in this group.\n` +
        ...
        try {
            ...
            const formattedStatus = status === "added" ? "✅ Added" : 
                                    status === "invite" ? "🔗 Invite link generated" : 
                                    "❌ Failed to add";
            ...
            if (inviteLink) {
                messageText += `\n📲 WhatsApp invite link (valid for 6 days):\n`;
                messageText += `${inviteLink}\n\n`;
                messageText += `ℹ️ Note: Share this invite link with ${cleanNumber} to add them to the group.`;
            }
            ...
            try {
                ...
                await sock.sendMessage(groupId, {
                    text: `❌ Error adding ${cleanNumber}: ${errorMessage}\n` +
        ...
        if (remainingContacts.length > 0 && scheduledRandomAdds.isActive) {
            ...
            try {
                await sock.sendMessage(groupId, {
                    text: `✅ Completed adding all scheduled contacts to the group!`
                });
            } catch (finalError) {
